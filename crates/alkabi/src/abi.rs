...
impl AbiDocument {
    ...
    /// TypeScript module source: the document exported `as const` (named
    /// `<contract>Abi` in lowerCamelCase), which is what gives a TS consumer
    /// literal types — a plain `.json` import widens them. Write this to an
    /// `abi.ts` file.
    #[cfg(feature = "extract")]
    pub fn to_ts(&self) -> String {
        let mut chars = self.contract.chars();
        let const_name = match chars.next() {
            Some(first) => first.to_lowercase().collect::<String>() + chars.as_str(),
            None => String::from("contract"),
        } + "Abi";
        format!(
