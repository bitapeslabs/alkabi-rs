...
println!("{}", abi.to_json());          // compact JSON
println!("{}", abi.to_json_pretty());   // pretty JSON (abi.json)
std::fs::write("abi.ts", abi.to_ts())?; // TypeScript module (abi.ts)
```

`to_ts()` renders the document as a TypeScript module — the ABI object
exported `as const` as `<Contract>Abi`, keeping the contract name's casing
verbatim (`Tortilla` → `TortillaAbi`, `AMMPool` → `AMMPoolAbi`):

```ts
// Generated by alkabi — do not edit.
export const TortillaAbi = { ... } as const;

export default TortillaAbi;
```

The `as const` form is what gives TypeScript consumers full literal types; a
plain `.json` import would widen them and break type inference.
